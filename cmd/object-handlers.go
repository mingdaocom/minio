...
func (api objectAPIHandlers) GetObjectHandler(w http.ResponseWriter, r *http.Request) {
	...
	if globalDomainNames != nil {
		url = strings.Join([]string{scheme, globalDomainNames[0], r.RequestURI}, "")
	} else {
		url = strings.Join([]string{scheme, r.Host, r.RequestURI}, "")
	}

	tokenarg := "&token=" + token
	baseUrl := strings.ReplaceAll(url, tokenarg, "")
	mingimageformat := &MingImageFormat{q: 90}
	for k, v := range query {
		if strings.Contains(k, "avinfo") {
			isAvinfo = true
			break
		}
		vlist := strings.Split(k, "|")
		if len(vlist) <= 1 {
			if strings.Contains(k, "watermark") {
				if v[0] != "" {
					formatImageUrl(mingimageformat, k+"="+v[0])
				} else {
					formatImageUrl(mingimageformat, k)
				}
			} else if strings.Contains(k, "imageView2") {
				formatImageUrl(mingimageformat, k)
			} else if strings.Contains(k, "imageMogr2") {
				formatImageUrl(mingimageformat, k)
			} else if strings.Contains(k, "vframe") {
				formatImageUrl(mingimageformat, k)
			}
		} else {
			//TODO 分割
			for _, value := range vlist {
				...
				if strings.Contains(k, "watermark") {
					if value != "" {
						formatImageUrl(mingimageformat, value+"="+value)
					} else {
						formatImageUrl(mingimageformat, value)
					}
				} else if strings.Contains(value, "imageView2") {
					formatImageUrl(mingimageformat, value)
				} else if strings.Contains(value, "imageMogr2") {
					formatImageUrl(mingimageformat, value)
				} else if strings.Contains(value, "vframe") {
					formatImageUrl(mingimageformat, value)
				}
			}
		}
	...
	if bucket == "mdoc" && isAvinfo == false {
		ok := TokenCheck(baseUrl, token)
		if ok {
			e := query.Get("e")
			baseUrl = strings.ReplaceAll(baseUrl, "&e="+e, "")
			if e != "" {
				nowTime := time.Now()
				eTime, _ := strconv.Atoi(e)
				if nowTime.Unix() > int64(eTime) {
					http.Error(w, "code=2,token check fail", 400)
					return
				}
			}
		} else {
		...
		etime := query.Get("e")
		equery := "&e=" + etime
		baseUrl = strings.ReplaceAll(baseUrl, equery, "")
		fmt.Println("get file base url ", baseUrl)
	}
	...
	if mingimageformat.state == true {
		cachePath, err = getImageCachePath(bucket, object, baseUrl)
		if err == nil && cachePath != "" {
			fmt.Println("use cache", baseUrl)
			cacheSize = GetFileSize(cachePath)
			//缓存出错
			if cacheSize == 0 {
				deleteCache(bucket, object)
			} else {
				useCache = true
			}
		}
	...
	if globalMingdaoFs.Mode == 3 {
		if isAvinfo || mingimageformat.state == true {
			if useCache == true {
				gr, err = globalMingdaoFs.S3GetObjectNInfo(ctx, bucket, object, rs, r.Header, readLock, opts, 2)
			} else {
				gr, err = globalMingdaoFs.S3GetObjectNInfo(ctx, bucket, object, rs, r.Header, readLock, opts, 1)
			}
			if err != nil {
				writeErrorResponse(ctx, w, toAPIError(ctx, err), r.URL, guessIsBrowserReq(r))
				return
			}
			defer os.Remove(gr.ObjInfo.path)
		} else {
			gr, err1 = globalMingdaoFs.S3GetObjectNInfo(ctx, bucket, object, rs, r.Header, readLock, opts, 0)
		}
	} else {
		gr, err1 = getObjectNInfo(ctx, bucket, object, rs, r.Header, readLock, opts)
	}

	if err1 != nil {
		writeErrorResponse(ctx, w, toAPIError(ctx, err1), r.URL, guessIsBrowserReq(r))
		return
	}
	defer gr.Close()
	objInfo := gr.ObjInfo
	...
	if isAvinfo {
		...
		if gr.ObjInfo.path != "" {
			mediainfo, _ = GetMediaInfo(gr.ObjInfo.path)
		} else {
			mediainfo, _ = GetMediaInfoByReader(gr)
		}
		//info, _ := json.Marshal(mediainfo)

		mingdaoVideo := formatMingdaoVideo(&mediainfo)

		result, _ := json.Marshal(mingdaoVideo)
	...
	if mingimageformat.state == true {
		if useCache == false {
			...
			if mingimageformat.watermark == "2" {
				fmt.Printf("no cache url %s path", baseUrl)
				if err := MingWaterMark(gr, mingimageformat); err != nil {
					if mingimageformat.ignoreError == 1 {
						mingimageformat.state = false
					} else {
						http.Error(w, err.Error(), 400)
						return
					}
				} else {
					objInfo.Size = gr.ObjInfo.Size
				}
			} else {
				fmt.Println("start ming pic")
				if err := MingPicService(gr, mingimageformat); err != nil {
					if mingimageformat.ignoreError == 1 {
						mingimageformat.state = false
					} else {
						http.Error(w, err.Error(), 400)
			...
			fmt.Printf("save url %s path %s", baseUrl, gr.tmppath)
			saveImageCache(gr.tmppath, bucket, object, path.Ext(gr.tmppath), baseUrl)
		} else {
			objInfo.Size = cacheSize
			gr.tmppath = cachePath
		}
		//这里重新设置下画完图的mime，兼容format参数
		mimetype := mime.TypeByExtension(path.Ext(gr.tmppath))
		if mingimageformat.format == "png" {
			mimetype = "image/png"
		} else if mingimageformat.format == "jpg" {
			mimetype = "image/jpeg"
		}
		objInfo.ContentType = mimetype
	}
...
// This implementation of the GET operation retrieves object. To use GET,
// you must have READ access to the object.
func (api objectAPIHandlers) GetObjectHandler_v1(w http.ResponseWriter, r *http.Request) {
