...
func (xl xlObjects) putObjectDis(ctx context.Context, bucket string, object string, r *PutObjReader,opts ObjectOptions) (objInfo ObjectInfo, err error){
	data := r.Reader
	...
	for partIdx := 1; ; partIdx++ {
		...
		fmt.Println("curPartSize   ",curPartSize)
		if err != nil {
			logger.LogIf(ctx, err)
			return ObjectInfo{}, toObjectErr(err, bucket, object)
		}
