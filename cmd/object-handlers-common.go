...
func checkPreconditions(ctx context.Context, w http.ResponseWriter, r *http.Request, objInfo ObjectInfo) bool {
	...
	if _,ok:=urlQuery["attname"];ok{
		w.Header().Set("Content-Disposition", "attachment")
		if urlQuery.Get("attname")!=""{
			w.Header().Set("Content-Disposition","attachment; "+ "filename=\""+urlQuery.Get("attname")+"\"; "+"filename*=utf-8' '"+url.QueryEscape(urlQuery.Get("attname")))
		}
	}
