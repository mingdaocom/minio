...
func GetMediaInfo(fname string) (MediaInfo, error) {
	...
	for _, v := range minfo.File.Tracks {
		if v.Type == "General" {
			...
			if len(v.Frame_rate) > 0 {
				general.Frame_rate = v.Frame_rate[0]
			}
		} else if v.Type == "Video" {
			video.Width, _ = strconv.Atoi(v.Width[0])
			video.Height, _ = strconv.Atoi(v.Height[0])
...
func GetMediaInfoByReader(gr io.Reader) (MediaInfo, error) {
	info := MediaInfo{}
	rand.Seed(time.Now().UnixNano())
	tmppath := globalTmpPrefix + "/tmp/" + strconv.Itoa(int(rand.Int()))
